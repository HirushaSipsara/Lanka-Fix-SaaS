 ...
 *
 * <p>Handles review creation and retrieval, plus rating recalculation.
 *
 * <p>SCRUM-94: Enforces strict ownership validation —
 * only the seeker who originally posted the service request can submit a review.
...
public class ReviewService {
     ...
     * </ul>
     *
     * @param reviewerId the ID of the authenticated user attempting to submit the review
    ...
    public ReviewResponse createReview(Long reviewerId, ReviewCreateRequest request) {
        ...
        // AC1/AC2 — Ownership Validation: Verify the reviewer is the original seeker of this request
        if (!serviceRequest.getSeeker().getId().equals(reviewerId)) {
            throw new UnauthorizedException("You do not have permission to review this job. Only the seeker who posted this request can submit a review.");
        }

        // Validate request is COMPLETED
        if (serviceRequest.getStatus() != RequestStatus.COMPLETED) {
            throw new BadRequestException("Reviews can only be submitted for completed requests. Current status: " + serviceRequest.getStatus());
        }

        // Check reviewer hasn't already reviewed this request
        ...
        // Determine the reviewee (the assigned worker)
        User reviewee = serviceRequest.getAssignedWorker();
        if (reviewee == null) {
    ...
    public List<ReviewResponse> getReviewsForWorker(Long workerId) {
        List<Review> reviews = reviewRepository.findByRevieweeId(workerId);
    ...
    public double getAverageRating(Long workerId) {
        Double avg = reviewRepository.findAverageRatingByRevieweeId(workerId);
